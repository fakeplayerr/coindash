...
func _physics_process(delta):
	...
	var direction = Vector2(input_dir.x, min(input_dir.y, 0)) # Allow only upward movement, not downward
	
	# Apply constant forward motion (upward in screen coordinates)
	if direction == Vector2.ZERO:
		# Moving straight ahead
		velocity.y = -forward_speed
		velocity.x = lerp(velocity.x, 0.0, 0.1)
	else:
		# Moving with input direction
		velocity.x = direction.x * speed
		velocity.y = -forward_speed + (direction.y * speed)
		
		# Tilting the car based on horizontal movement
		sprite.rotation_degrees = -direction.x * 15.0  # More noticeable tilt (in degrees)
	
	# Move the character
	move_and_slide()
	
	# Ensure the car stays within horizontal screen boundaries only
	position.x = clamp(position.x, wall_padding, screen_width - wall_padding)
	
	# No vertical clamping - allow player to move forward freely
	# This lets the camera follow properly
